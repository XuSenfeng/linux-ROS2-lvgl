...
{
    Game_Mem_Init();
    int weight = lv_obj_get_width(lv_scr_act()) > lv_obj_get_height(lv_scr_act()) ? lv_obj_get_height(lv_scr_act()) : lv_obj_get_width(lv_scr_act());
    ...
    lv_btnmatrix_set_map(ui_GameMem_BtnM, Game_Mem.btnm_map);
    lv_obj_set_style_text_font(ui_GameMem_BtnM, &ui_font_heiti24, 0);
    ...
    lv_obj_set_size(ui_GameMem_BtnM,weight,weight);
...
void ui_game_memory_button(lv_obj_t * ui_HomeScreen, lv_obj_t * ui_AppIconContainer, int x, int y, void (*button_cb)(lv_event_t *)){
    ...
    lv_obj_set_x(ui_GameMemBtn, x);
    lv_obj_set_y(ui_GameMemBtn, y);
    lv_obj_add_flag(ui_GameMemBtn, LV_OBJ_FLAG_SCROLL_ON_FOCUS);
    lv_obj_remove_flag(ui_GameMemBtn, LV_OBJ_FLAG_SCROLLABLE);
    ...
    lv_obj_add_event_cb(ui_GameMemBtn, button_cb, LV_EVENT_CLICKED, "GameMemoryPage");
}